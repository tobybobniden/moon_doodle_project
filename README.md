...
### 基本規則
- **月相值**：0-7，表示 8 個月相（🌑 🌒 🌓 🌔 🌕 🌖 🌗 🌘）
- **棋盤**：24 個連接的節點組成的圖形結構 (預設 Large Map)
- **玩家**：2 名玩家輪流落子
- **得分方式**：
  - **同象配對** (相同月相相鄰)：+1 分
  - **互補配對** (相差 4 的月相相鄰)：+2 分
...
### 版本 2.0 - 策略感知與硬體優化
- ✅ **分數感知 (Score Awareness)**：AI 輸入特徵擴增至 29 維，包含當前雙方分數，解決「領先時亂下」的問題。
- ✅ **混合獎勵機制 (Hybrid Reward)**：引入 `0.8 * 勝負 + 0.2 * 分差` 的獎勵函數，鼓勵 AI 不僅要贏，還要贏得高分。
...
- **FinalOutcome**: 贏 (+1), 輸 (-1), 平 (0)
- **ScoreDiff**: (我方分 - 對手分) / 最大可能分差

這迫使 AI 在確保勝利的前提下，盡可能擴大分差，從而學會更像人類高手的「貪心」策略。

---

## 環境設定與故障排除 (WSL2 + GPU)

本專案已針對 WSL2 (Windows Subsystem for Linux) 搭配 NVIDIA RTX 50 系列顯卡進行優化。

### 關鍵依賴
- **TensorFlow Nightly**: 必須使用 `tf-nightly` 以支援最新的 CUDA 架構 (Compute Capability 12.0+)。
- **CUDA Toolkit**: 建議使用 WSL2 內的 Docker 或 Conda 環境管理。

### 常見問題修復

#### 1. GPU 記憶體不足 (OOM)
若遇到 `OOM` 錯誤，請在 `Moon_Game_Launcher.ipynb` 中執行：
```python
from src.game_utils import clear_gpu_memory
clear_gpu_memory()
```
這會強制釋放 TensorFlow 佔用的 VRAM。

#### 2. 訓練速度慢 / JIT 編譯延遲
RTX 50 系列顯卡在首次執行時需要進行 JIT 編譯 (PTX -> Binary)。
- **現象**：第一次 `predict` 或 `train` 非常慢。
- **解法**：這是正常的。Launcher 中包含 `benchmark_gpu()` 函數，可以預熱 GPU 並確認加速效果。正常情況下，矩陣運算應有 5x-10x 的加速。

#### 3. 路徑問題
專案已移除對 Windows 路徑 (`C:\...`) 的依賴，全面改用 Linux 相對路徑。請確保在 WSL2 環境中執行，不要直接在 Windows CMD 中運行。
